      ...
      <div class="row g-2 align-items-center">
        <div class="col">
          ...
          <div class="text-muted mt-1">
            {{ $coreTeams->firstItem() ?? '0' }}-{{ $coreTeams->lastItem() ?? '0' }} dari
            {{ $coreTeams->total() }}
            pengurus inti
          </div>
        </div>
        <div class="col-auto ms-auto d-print-none">
          <div class="btn-list d-flex">
            ...
            <a href="#" class="btn btn-primary d-sm-none btn-icon" data-bs-toggle="modal" data-bs-target="#modalAdd"
              aria-label="Tambah Pengurus Inti">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-plus" width="24"
                height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
      ...
      <div class="row g-2 align-items-center">
        ...
        <div class="col-auto mt-3">
          @if (isParamsExist($allowedParams))
            <a href="{{ route('admin.dashboard.core-teams.index') }}" class="btn btn-outline-danger btn-icon"
      ...
      <div class="row">
        <div class="col">
          <div class="card">
                  ...
                  @foreach ($coreTeams as $coreTeam)
                    <tr>
                      ...
                      <td class="text-muted">
                        ...
                        {{ formatDate($coreTeam->created_at, 'd F Y H:i') }}
                      </td>
                      <td>
                        <div class="d-flex justify-content-center">
                          ...
                          <div class="text-muted dropdown-menu dropdown-menu-end">
                            <a class="dropdown-item"
                              href="{{ route('admin.dashboard.core-teams.edit', $coreTeam->id) }}">
                            ...
                            <button class="dropdown-item btn-action-delete"
                              data-action="{{ route('admin.dashboard.core-teams.destroy', $coreTeam->id) }}"
                    ...
                    <tr class="text-center">
                      <td colspan="99">
                        <div class="empty bg-transparent" style="height: 500px;">
                          ...
                          <p class="empty-title">Pengurus inti tidak ditemukan</p>
                          <p class="empty-subtitle text-muted">
                            Coba sesuaikan pencarian atau filter untuk menemukan apa yang anda
                            cari.
                          </p>
                          <div class="empty-action">
                            <a href="{{ route('admin.dashboard.core-teams.index') }}" class="btn btn-outline-danger">
            ...
            @if ($coreTeams->perPage() < $coreTeams->total())
              <div class="mt-3 ms-3">
                {{ $coreTeams->withQueryString()->onEachSide(1)->links('pagination.custom') }}
              </div>
            @endif
          </div>
        </div>
            ...
            <div class="row mb-3">
              <div class="col">
                ...
                <input type="text" class="form-control @error('position') is-invalid @enderror" name="position"
                  placeholder="Masukkan divisi" value="{{ old('position') }}" />
                @error('position')
            ...
            <div class="row mb-3">
              <div class="col">
                ...
                <input type="number" class="form-control @error('order') is-invalid @enderror" name="order"
                  placeholder="Masukkan urutan" value="{{ old('order') }}" />
                @error('order')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
